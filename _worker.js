...
function parseAddr(addr, defaultPort = '443') {
    ...
    if (parts.length > 2) {
        return {
            host: value,
            port: defaultPort,
            address: formatAddr(value, defaultPort)
        };
    }
    return {
        host: value,
        port: defaultPort,
        address: `${value}:${defaultPort}`
    };
}
...
function splitComment(line) {
    ...
    if (idx >= 0) return {
        main: line.substring(0, idx).trim(),
...
function checkRequestAuth(request, url, config) {
    ...
    const {
        ...
        qKey,
        cKey
    } = getAuthCandidateFromRequest(request, url);
...
async function handleAPIRequest(url, request, env, config) {
    if (POST_ONLY_ROUTES.has(url.pathname) && request.method !== 'POST') return new Response('Method Not Allowed', {
        status: 405
    });
    const handler = API_ROUTES[url.pathname];
    return handler ? await handler(url, request, env, config) : new Response('Not Found', {
        status: 404
    });
}
...
async function handleGetPool(url, env) {
    ...
    if (!isWritablePoolKey(poolKey)) return badRequest({
        success: false,
        error: '无效的池名称'
    });
    const pool = await env.IP_DATA.get(poolKey) || '';
    const count = pool.trim() ? pool.trim().split('\n').length : 0;
    if (onlyCount) return jsonResponse({
        count
    });
    return jsonResponse({
        pool,
...
async function handleSavePool(request, env) {
    ...
    if (!newIPs && mode !== 'remove') return badRequest({
        success: false,
        error: '没有有效IP'
    });
...
async function handleLoadRemoteUrl(request) {
    const body = await readJsonBody(request);
    if (!body || !body.url) return badRequest({
        success: false,
        error: '缺少URL'
    });
...
async function handleDeleteRecord(url, config) {
    ...
    return await deleteDNSRecord(cfConfig, id) ? jsonResponse({
        success: true
    }) : jsonResponse({
        success: false,
        error: 'CF API 删除失败'
...
async function handleAddARecord(request, config) {
    ...
    if (target.mode === 'TXT') {
        ...
        if (records === null) return jsonResponse({
            success: false,
            error: CF_ERROR_MSG
        });
        const record = records?.[0] || null;
        const currentIPs = record ? parseTXTContent(record.content) : [];
        if (currentIPs.includes(addr)) return jsonResponse({
            success: false,
            error: 'IP已存在于TXT记录'
        });
        currentIPs.push(addr);
        if (!await upsertTXTRecord(cfConfig, target.domain, record?.id, currentIPs)) return jsonResponse({
            success: false,
            error: 'CF API 保存TXT记录失败'
        });
        return jsonResponse({
            success: true,
            colo: check.colo,
...
async function handleMaintain(url, env, config) {
    ...
    return jsonResponse({
        ...res,
        allLogs: res.reports.flatMap(r => [...(r.logs || []), ...(r.txtLogs || [])])
    });
}
...
async function handleCreatePool(request, env) {
    ...
    if (poolKey.length > GLOBAL_SETTINGS.MAX_POOL_NAME_LENGTH || !/^pool_[\u4e00-\u9fa5a-zA-Z0-9_-]+$/.test(poolKey)) {
        return badRequest({
            success: false,
            error: `池名称只能包含中文、字母、数字、下划线、横杠，最长${GLOBAL_SETTINGS.MAX_POOL_NAME_LENGTH}字符`
        });
    }
    if (await env.IP_DATA.get(poolKey) !== null) return badRequest({
        success: false,
        error: '池已存在'
    });
    ...
    return jsonResponse({
        success: true,
        poolKey,
        displayName: rawName.replace(/^pool_/, '')
    });
}
...
async function handleRenamePool(request, env) {
    ...
    if (SYSTEM_POOL_KEYS.has(poolKey)) return badRequest({
        success: false,
        error: `不能重命名${getPoolDisplayName(poolKey)}`
    });
    if (poolKey !== 'pool' && validateEditablePoolKey(poolKey)) return badRequest({
        success: false,
        error: '池名称无效'
    });
    if (!displayName) return badRequest({
        success: false,
        error: '显示名称不能为空'
    });
    if (displayName.length > 40 || !/^[\u4e00-\u9fa5a-zA-Z0-9_\-\s]+$/.test(displayName)) return badRequest({
        success: false,
        error: '显示名称只能包含中文、字母、数字、空格、下划线、横杠，最长40字符'
    });
    const pool = await env.IP_DATA.get(poolKey);
    if (pool === null && poolKey !== 'pool') return badRequest({
        success: false,
        error: '池不存在'
    });
    const poolNames = await readPoolDisplayNames(env);
...
async function handleDeletePool(url, env) {
    ...
    if (DELETABLE_PROTECTED_POOL_KEYS.has(poolKey) || validateEditablePoolKey(poolKey)) return badRequest({
        success: false,
        error: `不能删除${getPoolDisplayName(poolKey)}`
    });
    if (await env.IP_DATA.get(poolKey) === null) return badRequest({
        success: false,
        error: '池不存在'
...
async function handleRestoreFromTrash(request, env) {
    ...
    if (!Array.isArray(ipsToRestore)) return badRequest({
        success: false,
        error: 'ips 必须是数组'
    });
    if (!isUserPoolKey(targetPool)) return badRequest({
        success: false,
        error: '无效的目标池'
    });
    if (ipsToRestore.length === 0) return badRequest({
        success: false,
        error: '没有选择IP'
    });
    ...
    return jsonResponse({
        ...
        restored,
        restoredByPool,
        message: restoreToSource ? `已恢复 ${restored} 个IP到源IP库` : `已恢复 ${restored} 个IP到 ${targetPool}`
    });
}
...
async function handleGetConfig(config) {
    return jsonResponse({
        success: true,
        config: getEditableConfig(config)
    });
}
...
function parseDomainPort(input, defaultPort = '443') {
    ...
    if (input.startsWith('[')) {
        ...
        return {
            domain,
            port: match ? match[1] : defaultPort
        };
    }
...
function normalizeTargetMode(value) {
    return String(value || 'A').trim().toUpperCase() === 'TXT' ? 'TXT' : 'A';
}
...
function normalizeTargetConfig(target) {
    ...
    return {
        ...
        exitFilter: normalizeExitFilter(target.exitFilter),
        country: String(target.country || '').trim().toUpperCase(),
        asn: normalizeAsnValue(target.asn),
...
function normalizeZoneConfig(zone) {
    ...
    return {
        ...
        zoneId,
        apiKey,
        label: String(zone.label || baseDomain || name || zoneId || '未命名').trim()
    };
}
...
function parseIPLine(line) {
    ...
    const isValidIP = ip => ip.split('.').every(o => {
        const n = Number(o);
        return n >= 0 && n <= 255;
    });
    const isValidPort = p => {
...
async function resolveDomainRecords(domain, config) {
    ...
    const records = [
        ...aRecords.filter(a => a.type === 1 && a.data).map(a => ({
            type: 'A',
            ip: a.data
        })),
        ...aaaaRecords.filter(a => a.type === 28 && a.data).map(a => ({
            type: 'AAAA',
            ip: a.data
        }))
    ];
...
async function resolveTXTRecord(domain, config) {
    try {
        ...
        return {
            raw: rawData.replace(/^"|"$/g, ''),
            ips: parseTXTContent(rawData)
        };
    } catch (e) {
        return {
            raw: '',
...
function normalizeAsnValue(value) {
    return normalizeTextValue(value).replace(/^AS/i, '');
}
...
function normalizeCheckResult(data, requestedAddr = '') {
    ...
    return {
        ...
        exits,
        ipInfo,
        asn: joinMetaValues(exits.map(item => item.asn)),
        country: joinMetaValues(exits.map(item => item.country)),
        stack,
...
function targetMetaMatchesStoredEntry(entry, target) {
    ...
    return targetMetaMatchesResult({
...
async function batchCheckIPs(ipList, checkFn, config, useBackupApi = false) {
    ...
    const effectiveCheckFn = (useBackupApi && config.checkApiBackup) ?
        (addr) => checkProxyIPOnce(normalizeCheckAddr(addr), config.checkApiBackup).then(r => r ?? {
            success: false
        }) : checkFn;

    const checkSettled = await Promise.allSettled(ipList.map(addr => effectiveCheckFn(addr)));
    const checkResults = checkSettled.map((r, i) => r.status === 'fulfilled' ? normalizeCheckResult(r.value, ipList[i]) : normalizeCheckResult({
        success: false
    }, ipList[i]));
...
async function maintainARecords(env, target, addLog, report, poolKey, checkFn, config, budget) {
    ...
    // 2. 盲测并挑选新节点 (双IP并发批测优化版)
    // 2.1 先筛选出符合基础格式的有效候选名单
    const validCandidates = [];
    for (const item of candidates) {
        const ipPort = extractIPKey(item);
        const parsed = parseAddr(ipPort, target.port);
        if (ipPort && parsed.port === target.port && !readyNodes.some(n => n.host === parsed.host)) {
    ...
    // 2.2 每次取出 2 个 IP 组合成一批进行测试
    const BATCH_SIZE = 2; // 根据接口特性，每次合并测 2 个 IP
    for (let i = 0; i < validCandidates.length; i += BATCH_SIZE) {
        ...
        const ipString = batch.map(b => b.ipPort).join(','); // 用逗号拼接
        ...
        let apiResponses = [];
        try {
            const rawRes = await checkFn(ipString);
            apiResponses = Array.isArray(rawRes) ? rawRes : [rawRes];
        } catch (e) {
            addLog(`  ⚠️ 批量测活请求失败`);
        }
        ...
        for (let j = 0; j < batch.length; j++) {
            ...
            const node = batch[j];
            const rawNodeRes = apiResponses.find(r => r && (r.proxyip === node.ipPort || r.ip === node.ipPort)) || apiResponses[j] || {
                success: false
            };

            const result = normalizeCheckResult(rawNodeRes, node.ipPort);
            ...
            if (matches) {
                ...
                addLog(`  ✅ [批测] 通过待上线: ${node.ipPort} - ${result.colo}`);
            } else {
                if (!result.success) {
                    const removed = removePoolEntry(poolList, node.ipPort);
                    ...
                    addLog(`  ❌ [批测] 抽中失效，移入垃圾桶: ${node.ipPort}`);
                }
            }
        }
    }
    ...
    if (readyNodes.length > 0) {
        ...
        for (; i < readyNodes.length; i++) {
            ...
            if (i < currentRecords.length) {
                // 覆盖旧记录
                batchPayload.puts.push({
                    id: currentRecords[i].id,
                    ...recordData
                });
            } else {
                // 新增记录
            ...
            report.added.push({
                ip: node.ipPort,
                colo: node.result.colo || 'N/A',
                time: node.result.responseTime || '-',
                country: node.result.country || 'null',
                asn: node.result.asn || 'null'
            });
        }

        // 清理多余的旧记录
        for (; i < currentRecords.length; i++) {
            batchPayload.deletes.push({
                id: currentRecords[i].id
            });
            report.removed.push({
                ip: formatAddr(currentRecords[i].content, target.port),
                reason: '清理冗余',
                colo: 'N/A',
                time: '-'
            });
...
async function maintainAllDomains(env, isManual = false, config) {
    ...
    const checkProxyIPCached = async (addr) => {
        const key = (addr || '').trim();
        if (!key) return normalizeCheckResult({
            success: false
        }, key);
    ...
    const poolEntries = poolSettled
        ...
        .filter(e => e !== null);
    poolEntries.forEach(([name, count]) => poolStats.set(name, {
        before: count,
        after: count
    }));
...
data - page = "dashboard"
onclick = "showPage('dashboard')" > 运行面板 < /button> <
    button class = "nav-tab"
data - page = "config"
onclick = "showPage('config')" > 配置中心 < /button> <
    /div>
...
id = "manual-add-ip"
class = "form-control"
placeholder = "手动添加IP到当前域名 (如: 1.2.3.4:443)" >
    <
    button class = "btn btn-success"
onclick = "manualAddIP()"
title = "添加IP到当前域名" > ➕ < /button> <
    /div> <
    /div>
...
style = "padding:6px 8px" > ➕ < /button> <
    button class = "btn btn-sm"
onclick = "renameCurrentPool()"
title = "重命名池"
style = "padding:6px 8px" > ✏️ < /button> <
    button class = "btn btn-sm"
onclick = "deleteCurrentPool()"
title = "删除池"
style = "padding:6px 8px" > 🗑️ < /button> <
    button class = "btn btn-sm"
onclick = "oneClickClean()"
title = "一键洗库"
style = "padding:6px 8px" > 🧹 < /button> <
    /div> <
    /div>
...
placeholder = "远程 CSV/TXT 链接"
style = "border-radius:8px; min-width: 160px;" >
    <
    input type = "text"
id = "remote-country"
class = "form-control form-control-sm"
placeholder = "国家(如 US,JP)"
style = "border-radius:8px; width: 100px;" >
    <
    input type = "text"
id = "remote-port"
class = "form-control form-control-sm"
placeholder = "端口(如 443)"
style = "border-radius:8px; width: 90px;" >
    <
    button class = "btn btn-sm btn-outline-primary"
onclick = "loadRemoteUrl()"
style = "white-space:nowrap" > 🌐极速拉取 < /button> <
    button class = "btn btn-sm btn-outline-secondary"
onclick = "loadCurrentPool()"
style = "white-space:nowrap" > 📂读池 < /button> <
    button class = "btn btn-sm btn-outline-danger"
onclick = "clearInput()"
style = "white-space:nowrap" > 🗑️清空 < /button>
...
style = "border-radius:12px;font-family:'SF Mono',monospace;font-size:12px" > < /textarea>
...
placeholder = "筛选" >
    <
    button class = "filter-help-btn"
onclick = "toggleFilterHelp()"
title = "筛选用法" > ? < /button> <
    button class = "btn btn-sm btn-outline-success"
onclick = "smartFilter('keep')"
title = "保留匹配的IP" > 保留 < /button> <
    button class = "btn btn-sm btn-outline-danger"
onclick = "smartFilter('exclude')"
title = "排除匹配的IP" > 排除 < /button> <
    button class = "btn btn-sm btn-outline-secondary"
onclick = "quickDeduplicate()"
title = "去除重复IP" > 去重 < /button> <
    /div> <
    div id = "filter-help"
class = "filter-help" >
...
class = "filter-preview" > 输入条件后会显示匹配数量。 < /div> <
    span class = "text-secondary small pool-stat"
title = "当前池中IP数量" > 📊 < span id = "pool-count" > 0 < /span></span >
    <
    /div> <
    /div>
...
onclick = "batchCheck()"
style = "border-radius:10px" > ⚡测速清洗 < /button> <
    button class = "btn btn-info flex-grow-1"
onclick = "saveByCountry()"
style = "border-radius:10px; color:white;" > 🗂️一键按国分发 < /button> <
    button class = "btn btn-success"
onclick = "saveToCurrentPool('append')"
style = "border-radius:10px" > 💾入库当前池 < /button> <
    /div>
...
onclick = "restoreSelected()" > ♻️恢复选中 < /button> <
    /div> <
    div class = "col-6" >
    <
    button class = "btn btn-outline-danger btn-sm w-100"
onclick = "clearTrash()" > 🗑️清空垃圾桶 < /button> <
    /div> <
    /div> <
    /div> <
    /div> <
    ...
    button class = "btn btn-sm btn-outline-primary"
onclick = "loadDomainPoolMapping()" > 🔄刷新 < /button> <
    /div> <
    div class = "table-responsive" >
    <
    ...
    <
    tr > < td colspan = "2"
class = "text-center text-secondary" > 加载中... < /td></tr >
    <
    /tbody> <
    /table> <
    /div> <
...
style = "width:0%; background:var(--success);" > < /div> <
    /div> <
    button id = "btn-maintain"
class = "btn btn-dark w-100"
onclick = "runMaintain()" > 🔧执行全部维护 < /button> <
    /div> <
    /div> <
    /div> <
    /div>
...
div class = "d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2" >
<
h6 class = "m-0 fw-bold" > ⚙️配置中心 < /h6> <
    div class = "config-toolbar" >
    <
    span id = "config-lock-hint"
class = "config-lock-hint" > 查看模式 < /span> <
    button id = "btn-edit-config"
class = "btn btn-sm btn-outline-primary"
onclick = "setConfigEditMode(true)" > 编辑配置 < /button> <
    button id = "btn-cancel-config"
class = "btn btn-sm btn-outline-secondary"
onclick = "cancelConfigEdit()"
style = "display:none" > 取消 < /button> <
    button id = "btn-save-config"
class = "btn btn-sm btn-success config-save-btn"
onclick = "saveAppConfig()"
style = "display:none" > 💾保存改动 < /button> <
    /div> <
    /div> <
...
id = "cfg-tg-enabled" >
    ...
    <
    label class = "field span-2" > < span > 检测 API < /span><small>主检测接口，支持 {proxyip} 占位符。</small > < input id = "cfg-check-api"
class = "form-control form-control-sm"
placeholder = "CHECK_API" > < /label> <
    label class = "field span-2" > < span > 备用检测 API < /span><small>复检或主接口失败时使用，可留空。</small > < input id = "cfg-check-api-backup"
class = "form-control form-control-sm"
placeholder = "CHECK_API_BACKUP" > < /label> <
    label class = "field" > < span > DoH API < /span><small>域名解析查询接口。</small > < input id = "cfg-doh-api"
class = "form-control form-control-sm"
placeholder = "DOH_API" > < /label> <
    label class = "field" > < span > 面板密钥 < /span><small>为空则关闭前端鉴权。</small > < input id = "cfg-auth-key"
class = "form-control form-control-sm"
placeholder = "AUTH_KEY" > < /label> <
    label class = "field" > < span > TG Bot Token < /span><small>开启 TG 通知时必填。</small > < input id = "cfg-tg-token"
class = "form-control form-control-sm"
placeholder = "TG_TOKEN" > < /label> <
    label class = "field" > < span > TG Chat ID < /span><small>通知接收账号或群组 ID。</small > < input id = "cfg-tg-id"
class = "form-control form-control-sm"
placeholder = "TG_ID" > < /label> <
    /div> <
    /div>
    ...
    button class = "btn btn-sm btn-outline-primary config-add-action"
onclick = "addZoneConfigRow()" > ➕添加权限配置 < /button> <
    /div> <
    div id = "zone-config-list"
class = "config-card-grid" > < /div> <
div id = "zone-edit-panel"
class = "config-edit-panel" > < /div> <
/div>
...
h6 class = "m-0 fw-bold" > 🧭管理域名 < /h6> <
    button class = "btn btn-sm btn-outline-primary config-add-action"
onclick = "addTargetConfigRow()" > ➕添加管理域名 < /button> <
    /div> <
    div id = "target-config-list"
class = "config-card-grid" > < /div> <
div id = "target-edit-panel"
class = "config-edit-panel" > < /div> <
/div> <
/div> <
/div> <
...
function getPoolName(key) {
    return poolDisplayNames[key] || POOL_NAMES[key] || key.replace('pool_', '') + '池';
}
...
function getDraftZones() {
    ...
    configDraft.zones = (Array.isArray(configDraft.zones) && configDraft.zones.length ? configDraft.zones : [{
            label: '配置1'
        }])
...
function renderZoneConfigRows(zones) {
    ...
    if (!list) return;
    const rows = (Array.isArray(zones) && zones.length ? zones : [{
        label: '配置1',
        baseDomain: '',
        zoneId: '',
        apiKey: ''
...
function addZoneConfigRow() {
    ...
    const zones = getDraftZones();
    showZoneEditor(zones.length, {
        label: '配置' + (zones.length + 1),
        baseDomain: '',
        zoneId: '',
        apiKey: ''
    });
...
function deleteZoneConfig(index) {
    ...
    configDraft.zones = (zones.length ? zones : [{
        label: '配置1'
    }]).map(normalizeDraftZone);
...
function renderTargetConfigRows(targets) {
    ...
    if (!list) return;
    const rows = (Array.isArray(targets) && targets.length ? targets : [{
        mode: 'A',
        zoneIndex: 0,
        prefix: '',
        port: '443',
...
function addTargetConfigRow() {
    if (!ensureConfigEditable()) return;
    showTargetEditor((configDraft?.targets || []).length, {
        mode: 'A',
        zoneIndex: 0,
        prefix: '',
        port: '443',
...
function parseAddrParts(addr) {
    ...
    async function checkIPWithInfo(addr) {
        ...
        return {
            ...
            exits: r.exits || [],
            proxyIP: r.proxyIP,
            portRemote: r.portRemote,
            ipInfo: r.ipInfo || null,
            asn: r.asn,
            country: r.country,
            stack: r.stack
        };
        ...
        try {
            ...
            const promises = Object.entries(groups).map(async ([poolKey, ipLines]) => {
                ...
                if (r.success) {
                    ...
                    log('  ❌ [' + poolKey.replace('pool_', '') + '池] 入库失败: ' + r.error, 'error');
                }
            });
        ...
        try {
            ...
            if (data.mode === 'TXT' && data.txtRecords && data.txtRecords.length > 0) {
                ...
                record.ips.forEach(ip => {
                    allRows.push(renderIPRow(
                        ip, \`<a href="javascript:deleteTxtIP('\${escapeJSString(record.id)}', '\${escapeJSString(ip.ip)}')" class="text-danger text-decoration-none small fw-bold">🗑️</a>\`
                    ));
                });
            }
    ...
    async function lookupDomain() {
        ...
        try {
            if (val.startsWith('txt@')) {
                ...
                // 显示在表格中（与地址记录探测统一模板）
                t.innerHTML = checkResults.map(r => renderIPRow(r, \`<button class="btn btn-sm btn-outline-primary" onclick="addToInput('\${escapeHTML(buildPoolLineFromCheckResult(r.ip, r))}')" title="添加到输入框">➕</button>\`)).join('');
    ...
    async function clearTrash() {
        ...
        try {
            const r = await apiFetch('/api/clear-trash', {
                method: 'POST'
            }).then(r => r.json());
            if (r.success) {
                log('✅ 垃圾桶已清空', 'success');
        ...
        try {
            ...
            if (validCount > 0) {
                const r = await apiFetch('/api/save-pool', {
                    method: 'POST',
                    body: JSON.stringify({
                        pool: validLines.join('\\n'),
                        poolKey: cleaningPool,
                        mode: 'replace'
                    })
                }).then(r => r.json());
    ...
    /script> <
    footer class = "container text-center text-secondary small py-3" > DDNS Pro· $ {
        version
    } < /footer> <
    /body> <
    /html>`;
